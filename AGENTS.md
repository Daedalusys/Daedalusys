...
- **Purpose**: 可复用的参数化配置模板(蓝图)渲染与安全应用:nginx 站点/反代、postgres 库/用户、redis ACL、haproxy 后端。数据经 `//go:embed` 编入二进制(`daedalus/core/cmd/daedalus-blueprint/blueprints/`,构建期由 `just blueprint-embed` 从 `daedalus/plugin/blueprint/blueprints/` 复制,源码侧为唯一事实源),渲染走 Go `text/template`,schema 校验经 `internal/blueprint` + `jsonschema-go` 预编译。
