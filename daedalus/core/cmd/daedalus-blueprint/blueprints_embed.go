...
//	拒绝,而蓝图源码侧位于 daedalus/plugin/blueprint/blueprints/(在 core Go
//	模块之外)。因此构建期经 `just blueprint-embed`(rsync)把蓝图数据复制到
//	本目录 blueprints/ 下,再经 `//go:embed all:blueprints/*` 嵌入二进制。
//	复制产物不入库(见 daedalus/core/.gitignore),源码侧
//	daedalus/plugin/blueprint/blueprints/ 仍是唯一事实源。
//
