...
//
// 路径解析说明(flaky 防线):policy 测试与 T3 的 TestPolicy_ObjectModel
// 同法——清空 DAEDALUS_POLICY_PATH、机器上若存在镜像生产路径则 Skip,
// 然后走 policy.ResolvePath() 的 DevRelPath 自 cwd 逐级上溯。本测试的
// cwd 是 daedalus/core/internal/objectmodel,上溯到仓库根命中
// daedalus/files/system/opt/daedalus/shared/policy.toml,与 go test 的
// 包目录机制绑定(不依赖 runtime.Caller,行为确定)。
//
...
func TestObjectModel_Drift(t *testing.T) {
	// --- (b) 腿输入:加载仓库真实 policy.toml(镜像 T3 的解析方式) ---
	t.Setenv(policy.EnvPolicyPath, "") // 清空显式注入口,强制走解析链
	if st, err := os.Stat(policy.ProductionPath); err == nil && !st.IsDir() {
		t.Skipf("本机存在镜像生产策略 %s,无法演练开发态回溯", policy.ProductionPath)
	}
	policyPath, err := policy.ResolvePath()
	if err != nil {
		t.Fatalf("DevRelPath 上溯未命中仓库 policy.toml: %v", err)
	}
	loaded, err := policy.Load(policyPath)
	if err != nil {
		t.Fatalf("真实 policy.toml 未通过自身校验: %v", err)
	}
